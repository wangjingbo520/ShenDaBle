...
public class BleManager {
    ...
    public void notify(BleDevice bleDevice,
        ...
        if (bleBluetooth == null) {
            callback.onNotifyFailure(bleDevice.getMac(),new OtherException("This device not connect!"));
        } else {
            bleBluetooth.newBleConnector()
                    .withUUIDString(uuid_service, uuid_notify)
                    .enableCharacteristicNotify(bleDevice.getMac(),callback, uuid_notify, useCharacteristicDescriptor);
        }
    }
    ...
    public void indicate(BleDevice bleDevice,
        ...
        BleBluetooth bleBluetooth = multipleBluetoothController.getBleBluetooth(bleDevice);
        if (bleBluetooth == null) {
            callback.onIndicateFailure(bleDevice.getMac(),new OtherException("This device not connect!"));
        } else {
            bleBluetooth.newBleConnector()
                    .withUUIDString(uuid_service, uuid_indicate)
                    .enableCharacteristicIndicate(bleDevice.getMac(),callback, uuid_indicate, useCharacteristicDescriptor);
        }
    }
